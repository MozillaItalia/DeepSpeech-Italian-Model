...
class EvalitaImporter(ArchiveImporter):
    ...
    def get_corpus(self):
        ...
        utterances = {}
        audios = []
        dev_wav_dir = os.path.join(self.origin_data_path, "development")
        train_wav_dir = os.path.join(self.origin_data_path, "training")
        dev_transcript_path = os.path.join(self.origin_data_path, self.archive_name, "development.txt")
        train_transcript_path = os.path.join(self.origin_data_path, self.archive_name, "training.txt")

        with open(train_transcript_path) as fin:
             for line in fin:                 
...
if __name__ == "__main__":
    ...
    evalita_importer = EvalitaImporter(corpus_name,archivie_url)
